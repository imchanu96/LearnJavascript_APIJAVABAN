...
// 		alert(selList.options[selList.selectedIndex].value);
		var nameStr = selList.options[selList.selectedIndex].value;
			imObj.innerHTML = "<img src= ./images/" 
			+ nameStr + ".PNG style= 'width:300px; height: 300px;'>";
// 		imObj.appendChild(imgObj);
	}
	function selStar() {
		var selList = document.getElementById("sel");
		var nameList = selList.options[selList.selectedIndex].value;
// 		alert(nameList);
		var name = "";
		for (var i = 0; i <= nameList.length; i++) {
			if (selList[i].selected) {
				name = name + selList[i].value + " ";
			}
		}
		alert(name);
	}
		...
		<tr>
			<td style="width:50px;">
				<select id="sel" size="4" multiple>
					...
					<option value="안지영">4)안지영</option>
				</select>
			</td>
			<td style="width:100px;">
				<span id="im"><!-- 여기에 선택한 연예인의 이미지를 출력한다 -->
